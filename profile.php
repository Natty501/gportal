...
if (isset($_GET['student_no']) && !empty($_GET['student_no'])) {
    ...
    $query = $con->prepare("SELECT fname, mname, lname, student_no, year_level, strand_course, avatar_choice, email, personal_email, about, last_activity 
                            FROM form 
                            WHERE student_no = ?");
    ...
    <div class="max-w-6xl mx-auto">
      ...
      <div class="flex flex-col md:flex-row items-center md:items-start gap-8">
        ...
        <div class="profile-info text-center md:text-left">
          <h2><?= $fullName = $user['fname'] 
    . (!empty($user['mname']) && $user['mname'] !== 'N/A' ? ' ' . ucfirst($user['mname']) : '') 
    . ' ' . ucfirst($user['lname']);?></h2>
          ...
          <div class="tab-pane" id="contact">
            <div class="info-section">
              ...
              <div class="info-grid grid-cols-1 md:grid-cols-2">
                ...
                <div class="info-value"><?= htmlspecialchars($user['personal_email'] ?? '') ?>
          </div>
